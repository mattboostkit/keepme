...
interface AboutPageData {
  ...
  glassManufacturerTitle: string;
  glassManufacturerDescription1: string;
  glassManufacturerDescription2: string;
  glassManufacturerImage?: SanityImageReference;
  glassManufacturerCtaText: string;
  glassManufacturerCtaUrl: string;
  ctaTitle: string;
...
function About() {
  ...
  const [timeline, setTimeline] = useState<CompanyTimeline[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pageResult = await fetchSanityData<AboutPageData>(
          '*[_type == "aboutPage"][0]'
        );
        setPageData(pageResult);
        
        // Fetch Company Values
        const valuesResult = await fetchSanityData<CompanyValue[]>(
          '*[_type == "companyValue"] | order(displayOrder asc)'
        );
        setCompanyValues(valuesResult);
        
        // Fetch Company Timeline
        const timelineResult = await fetchSanityData<CompanyTimeline[]>(
          '*[_type == "companyTimeline"] | order(displayOrder asc)'
        );
  ...
  return (
    <div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-4xl md:text-5xl font-sans font-semibold text-brand-plum mb-6">
                {pageData?.heroTitle?.split(' ').slice(0, -1).join(' ') || 'About'} <span className="text-brand-rose">{pageData?.heroTitle?.split(' ').slice(-1).join(' ') || 'KeepMe'}</span>
              </h1>
              <p className="text-lg text-brand-mauve leading-relaxed mb-12">
                {pageData?.heroDescription || 'We specialise in the creation, manufacture, and delivery of premium fragrance, home, and lifestyle products. Based in the UK, we are a family-run business with over 20 years of industry experience. We work closely with brands, license holders, distributors, and retailers to bring their visions to life — from initial concept through to final product delivery. Whether you need individual components or a retail-ready product, KeepMe is the trusted partner to support your project at every stage.'}
              </p>
              <div className="bg-white p-6 rounded-xl shadow-md">
                <h2 className="text-3xl md:text-4xl font-serif font-bold text-brand-plum mb-6">{pageData?.missionTitle?.split(' ').slice(0, -1).join(' ') || 'Our'} <span className="text-brand-rose">{pageData?.missionTitle?.split(' ').slice(-1).join(' ') || 'Mission'}</span></h2>
                <p className="text-lg text-brand-mauve leading-relaxed">
                  {pageData?.missionDescription || 'We\'re all about turning big ideas into high-quality products — and making the whole process feel easy and collaborative.'}
                </p>
              </div>
            </div>
            <div className="relative">
              <img
                ...
                alt={pageData?.heroImage?.alt || "KeepMe perfume bottles"}
              ...
              <div className="absolute -bottom-8 -left-8 bg-white rounded-xl p-6 shadow-lg">
                <div>
                  <p className="text-xl font-semibold">{pageData?.heroBadge1Text || 'Since 2004'}</p>
                  <p className="text-gray-600">{pageData?.heroBadge1Subtext || 'Crafting Excellence'}</p>
                </div>
              </div>
              ...
              <div className="absolute -bottom-8 -right-8 bg-white rounded-xl p-6 shadow-lg hidden md:block"> {/* Hide on small screens */}
                <div>
                  <p className="text-xl font-semibold">{pageData?.heroBadge2Text || '5M+ Units'}</p>
                  <p className="text-gray-600">{pageData?.heroBadge2Subtext || 'Produced Per Annum'}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-brand-plum mb-6">{pageData?.valuesTitle?.split(' ').slice(0, -1).join(' ') || 'Our'} <span className="text-brand-rose">{pageData?.valuesTitle?.split(' ').slice(-1).join(' ') || 'Values'}</span></h2>
            <p className="text-lg text-brand-mauve leading-relaxed">
              {pageData?.valuesSubtitle || 'The principles that guide our work and relationships'}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {companyValues.length > 0 ? (
              companyValues.map((value) => (
                <div key={value._id} className="bg-white p-8 rounded-2xl shadow-md hover:shadow-xl transition-shadow">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-brand-plum mb-6">{pageData?.journeyTitle?.split(' ').slice(0, -1).join(' ') || 'Our'} <span className="text-brand-rose">{pageData?.journeyTitle?.split(' ').slice(-1).join(' ') || 'Journey'}</span></h2>
            <p className="text-lg text-brand-mauve leading-relaxed">{pageData?.journeySubtitle || 'How KeepMe Lifestyle established itself as a key supplier in the fragrance and lifestyle sector'}</p>
          </div>
            ...
            <div className="relative max-w-7xl mx-auto pb-20">
              ...
              {timeline.length > 0 ? (
                timeline.map((event, index) => (
                  <div key={event._id} className="flex justify-start pt-10 md:pt-20 md:gap-10">
                    <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                      ...
                      <h3 className={`hidden md:block text-xl md:text-3xl lg:text-5xl font-bold font-serif md:pl-20 transition-colors duration-300 ${index === 0 ? 'text-brand-mauve dark:text-brand-mauve' : 'text-brand-mauve/50 dark:text-brand-mauve/50'}`}>{event.year}</h3>
                    </div>
                    <div className="relative pl-20 pr-4 md:pl-4 w-full">
                      <h3 className={`md:hidden block text-2xl mb-4 text-left font-bold font-serif transition-colors duration-300 ${index === 0 ? 'text-brand-mauve dark:text-brand-mauve' : 'text-brand-mauve/50 dark:text-brand-mauve/50'}`}>{event.year}</h3>
                      <div>
                        <h4 className="text-lg font-normal text-brand-rose dark:text-brand-rose mb-2">{event.title}</h4>
                        <p className="text-brand-mauve dark:text-neutral-300 text-base leading-relaxed">{event.description}</p>
                        {event.additionalContent && event.additionalContent.length > 0 && (
                          <div className="mt-2">
                            {event.additionalContent.map((block, blockIndex) => {
                              if (block._type === 'block' && block.listItem === 'bullet') {
                                return (
                                  <ul key={blockIndex} className="list-disc pl-5 text-brand-mauve dark:text-neutral-300 text-base leading-relaxed">
                                    <li>{block.children[0]?.text}</li>
                                  </ul>
                                );
                              } else if (block._type === 'block') {
                                return (
                                  <p key={blockIndex} className="text-brand-mauve dark:text-neutral-300 text-base leading-relaxed mt-2">
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative">
              <img
                src={pageData?.glassManufacturerImage?.asset ? urlFor(pageData.glassManufacturerImage).width(603).height(664).fit('crop').format('webp').url() : "https://cdn.sanity.io/images/tyzs5imn/production/d1b61dc82170aa5a21ee677f8ce6423223fad74f-603x664.webp"}
                alt={pageData?.glassManufacturerImage?.alt || "KeepMe Glass perfume bottle"}
            ...
            <div className="text-left">
              <h3 className="text-3xl md:text-4xl font-serif font-bold text-brand-plum mb-6">
                {pageData?.glassManufacturerTitle || 'Glass Manufacturer for the Fragrance and Lifestyle sector'}
              </h3>
              <p className="text-lg text-gray-700 mb-4 leading-relaxed">
                {pageData?.glassManufacturerDescription1 || 'At KeepMe Glass, we specialise in designing and manufacturing premium quality glass bottles and components for the fragrance, beauty, and lifestyle industries.'}
              </p>
              <p className="text-lg text-gray-700 mb-8 leading-relaxed">
                {pageData?.glassManufacturerDescription2 || 'We offer a full design and decoration service for customers looking for a high-end, bespoke finish, and a catalogue of hundreds of stock bottles, with short lead times with low MOQs and short lead times for those looking for a faster turnaround.'}
              </p>
              <a
                href={pageData?.glassManufacturerCtaUrl || "https://keepmeglass.co.uk/"}
                target="_blank"
              ...
              >
                {pageData?.glassManufacturerCtaText || 'Visit KeepMe Glass'}
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6">
          <div className="text-center">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-4">
              {pageData?.ctaTitle?.split(' ').slice(0, -1).join(' ') || 'Contact'} <span className="text-brand-mauve">{pageData?.ctaTitle?.split(' ').slice(-1).join(' ') || 'Us'}</span>
            </h2>
            <p className="text-lg text-gray-600">{pageData?.ctaDescription || 'Get in touch to discuss your project requirements'}</p>
